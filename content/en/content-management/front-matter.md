...
A [node](g) can pass front matter values down to its descendants. However, a cascaded value will not apply if the descendant already defines that field, or if a nearer ancestor has already cascaded a value for the same field.
...
The `_target` keyword allows you to target specific pages or [environments](g).  For example, to cascade a "color" parameter from the home page only to pages within the "articles" section, including the "articles" section page itself:
