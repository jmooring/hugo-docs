...
Before Hugo v0.56.0, custom [component](g) paths could only be configured by setting [`archetypeDir`][], [`assetDir`][], [`contentDir`][], [`dataDir`][], [`i18nDir`][], [`layoutDir`][], or [`staticDir`][] in the site configuration. Module mounts offer greater flexibility than these legacy settings, but
you cannot use both.
