...
`Identifiers`
: (`slice`) A slice containing the `id` attribute of each heading on the page. If so configured, will also contain the `id` attribute of each description term (i.e., `dt` element) on the page. See [configure Markup][].
...
[configure Markup]: /docs/reference/configuration/markup/#parserautodefinitiontermid
