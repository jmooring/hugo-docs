...
In some rare use cases, you may need to defer the execution of a template until after all sites and output formats have been rendered. One such example could be [TailwindCSS](/docs/reference/functions/css/tailwindcss/) using the output of [hugo_stats.json](/docs/reference/configuration/build/) to determine which classes and other HTML identifiers are being used in the final output:
