...
When installing themes as Git submodules or archives, compose a theme from multiple themes by defining the `theme` key as an array:

{{< code-toggle file=hugo >}}
theme = ['my-shortcodes', 'my-theme', 'other-theme']
{{< /code-toggle >}}

When installing themes as Hugo modules, define multiple imports:

{{< code-toggle file=hugo >}}
[[module.imports]]
path = 'github.com/user/my-shortcodes'

[[module.imports]]
path = 'github.com/user/my-theme'

[[module.imports]]
path = 'github.com/user/other-theme'
{{< /code-toggle >}}

In both cases, Hugo resolves [components](g) by searching the project first, then each theme in the order listed.

Each constituent theme can itself define additional theme imports in its own `hugo.toml`, enabling theme inheritance.
