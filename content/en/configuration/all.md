...
: (`string`) The project's default language key, conforming to the syntax described in [RFC 5646]. This value must match one of the defined language keys.
...
disableLanguages
: (`[]string]`) A slice of language keys representing the languages that should be disabled during the build process.
...
ignoreVendorPaths
: (`string`) A [glob](g) pattern specifying module paths to exclude from the `_vendor` directory.
...
titleCaseStyle
: (`string`) The capitalization rules to follow when Hugo automatically generates a section title, or when using the [`strings.Title`] function. One of `ap`, `chicago`, `go`, `firstupper`, or `none`. Default is `ap`. See&nbsp;[details](#title-case).
