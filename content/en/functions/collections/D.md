...
params:
  functions_and_methods:
    returnType: '[]int'
    signatures: [collections.D SEED N MAX]
---

{{< new-in 0.149.0 />}}

The `collections.D` function returns a slice of `N` sequentially ordered unique random integers in the half-open [interval](g) [0, `MAX`) using the provided `SEED` value. This function implements J. S. Vitter's Method&nbsp;D[^1] for sequential random sampling, a fast and efficient algorithm for this task.
...
A common use case is the selection of random pages from a page collection. For example, to render an unordered list of 5 random pages using the current time as the seed value:

```go-html-template
<ul>
  {{ $p := site.RegularPages }}
  {{ range collections.D now.Unix 5 ($p | len) }}
    {{ with (index $p .) }}
