...
In terms of PDF generation, we utilized [custom output formats](/docs/reference/configuration/output-formats/) to produce intermediary document representations, which are consumed by our custom tool transforming them to TeX documents, which are finally used to produce PDF files.
